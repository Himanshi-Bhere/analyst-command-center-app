    ...
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card className="lg:col-span-2"><H2>Appearance</H2><div className="text-[12.5px] text-muted mt-1">Each theme is a different look — background, card material, fonts, corners and spacing. Applies instantly.</div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-3">{THEMES.map((t) => <button key={t.id} onClick={() => setSettings({ theme: t.id, themeChosen: true })} className={cn('text-left rounded-xl border overflow-hidden transition-all', (s.theme || 'midnight') === t.id ? 'border-accent ring-2 ring-accent/30' : 'border-line hover:border-line2')}><div className="h-20 relative" style={{ background: t.preview }}><div className="absolute left-3 top-3 right-3 h-3 rounded-sm opacity-80" style={{ background: t.id === 'paper' ? '#fff' : 'rgba(255,255,255,0.12)', border: '1px solid rgba(127,127,127,0.25)' }} /><div className="absolute left-3 top-8 w-1/2 h-3 rounded-sm" style={{ background: t.swatch[1] }} /></div><div className="p-2.5"><div className="text-[13px] font-semibold flex items-center justify-between">{t.name}{(s.theme || 'midnight') === t.id && <span className="text-accent-glow text-[11px]">Active</span>}</div><div className="text-[11px] text-muted mt-0.5 leading-snug">{t.desc}</div></div></button>)}</div>
        </Card>
        <Card className="lg:col-span-2"><H2>Notifications</H2>
          <div className="text-[12.5px] text-muted mt-1">The bell in the top bar always shows in-app nudges (open tasks, must-do in the evening, overdue, GitHub pushes, showcase days, interviews, streak). Browser alerts additionally pop up on your desktop / phone (installed app) for urgent and warning items — once per item per day.</div>
          <div className="flex flex-wrap items-center gap-3 mt-3">
            <button className={cn('btn-xs', state.notify?.browser ? 'btn-subtle' : 'btn-primary')} onClick={async () => { const { requestNotifyPermission } = await import('../engine/notify'); const p = await requestNotifyPermission(); state.setNotify({ browser: p === 'granted' }) }}>{state.notify?.browser ? 'Browser alerts: on' : 'Enable browser alerts'}</button>
            {state.notify?.browser && <button className="btn-ghost btn-xs" onClick={() => state.setNotify({ browser: false })}>Turn off</button>}
            <button className="btn-ghost btn-xs" onClick={() => state.clearDismissed()}>Restore dismissed notifications</button>
            {typeof Notification !== 'undefined' && Notification.permission === 'denied' && <span className="text-[11.5px] text-warn">Blocked in the browser — allow notifications for this site in the address-bar settings.</span>}
          </div>
        </Card>
